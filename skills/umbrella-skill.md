...
- **Any structural move from Section 7.3**: three-sentence preamble before the point, closing summary paragraph, a "Conclusion" heading, *Furthermore / Moreover / In conclusion* paragraph openers, triadic adjectives, round-numbered lists when the real count is 4 or 7, exclamation marks (anywhere, including paraphrased quotes), emojis in body text, em dashes (the `—` character, any use; reach for a colon, comma, period or parentheses instead. En dashes for numeric ranges are fine), Oxford commas (no comma before the final *and* / *or* in a list of three or more), heavy semicolon use (a tight 600–900 word proposal should have zero semicolons, a medium 1200–1800 word proposal one or two at most. Semicolons used as em-dash substitutes are the typical AI-edit tell).
