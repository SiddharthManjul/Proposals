...
import { db } from "@/db";
import { comments, proposals } from "@/db/schema";
...
async function main() {
  ...
  await db.execute(
    sql`TRUNCATE TABLE comments, proposals RESTART IDENTITY CASCADE`
  );

  console.log(`→ Seeding ${SAMPLE_PROPOSALS.length} proposals…`);
  let totalComments = 0;
    ...
    for (const top of p.discussion) {
      const [topRow] = await db
        ...
        .values({
          ...
          body: top.body,
          createdAt: top.date
            ? new Date(`${top.date}T12:00:00Z`)
            : baseDate,
        })
        .returning({ id: comments.id });
      idByOriginal.set(top.id, topRow.id);
      totalComments += 1;
  ...
  console.log(
    `✓ Seed complete: ${SAMPLE_PROPOSALS.length} proposals, ${totalComments} comments.`
  );
  process.exit(0);
}
